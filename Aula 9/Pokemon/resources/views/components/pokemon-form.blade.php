<div x-data="{ open: $store.modal?.open ?? false }" x-bind:open="$store.modal.open" class="fixed inset-0 z-50 overflow-y-auto" x-show="open" x-transition x-on:keydown.escape.window="$store.modal.open = false" x-init="$watch('open', value => $store.modal.open = value)">
    <div class="flex min-h-full items-center justify-center p-4">
        <div class="fixed inset-0 bg-black/50 backdrop-blur-sm" x-on:click="$store.modal.open = false"></div>
        
        <div class="relative w-full max-w-2xl transform rounded-2xl bg-slate-900/95 border-2 border-emerald-500/50 p-8 shadow-2xl backdrop-blur-xl animate-in fade-in zoom-in duration-300" x-on:keydown.escape.window="$store.modal.open = false">
            <div class="flex items-center justify-between mb-8">
                <h2 class="text-3xl font-bold bg-gradient-to-r from-emerald-400 to-teal-500 bg-clip-text text-transparent">
                    Novo Pokémon
                </h2>
    <button x-on:click="$store.modal.open = false" class="p-2 rounded-xl hover:bg-slate-800/50 transition-all duration-200">
            ...
            <form action="{{ route('pokemon.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <button type="button" x-on:click="$store.modal.open = false" class="flex-1 p-4 rounded-xl border-2 border-slate-600/50 bg-slate-800/50 text-slate-300 hover:border-emerald-500 hover:bg-emerald-500/10 transition-all duration-200 font-semibold">
                        Cancelar
                    </button>
                </div>
            </form>
